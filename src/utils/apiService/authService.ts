...
mock.onGet(/^\/api\/v1\.0\/auth\/.*/).reply((config: any) => {
  try {
    console.log("get");
...
mock.onPost(/^\/api\/v1\.0\/auth\/.*/).reply((config: any) => {
  try {
    let results;
    console.log("post");
    console.log(config.url);
...
mock.onPut(/^\/api\/v1\.0\/auth\/.*/).reply((config: any) => {
  try {
    console.log("put");
    console.log(config.url);
    let results;
...
mock.onDelete(/^\/api\/v1\.0\/auth\/.*/).reply((config: any) => {
  try {
    console.log("delete");
    console.log(config.url);
    let results;
...
export const latestTermsCheck = async () => {
  const api = APIBuilder.get(`${baseUri}/terms/latest`)
    .withCredentials(true)
    .build();
  const { data } = await api.call<Response<AuthResponse>>();
  return data;
};
